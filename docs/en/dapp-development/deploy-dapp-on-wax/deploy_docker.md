...
5. Next, open **Makefile**. This file contains the scripts to run `cleos` and the WAX Docker Development image.
    ...
    <strong>Note:</strong> `NODEOS_URL` is the only optional parameter. Its default value is the mainnet deployment address [chain-api-url](/en/wax-infra/#public-and-free-api-service-providers).  
    {: .label .label-yellow }
